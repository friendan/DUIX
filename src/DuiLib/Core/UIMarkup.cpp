...
bool CMarkup::LoadFromFile(LPCTSTR pstrFilename, int encoding)
{
    ...
    BYTE* pByte = NULL;
    DWORD dwSize = 0;
    if( !CPaintManagerUI::LoadResourceData(pstrFilename, &pByte, &dwSize) )
...
{
	// 允许字母数字、下划线、冒号、连字符（如 header-align）
	while( *pstr != _T('\0') && (*pstr == _T('_') || *pstr == _T(':') || *pstr == _T('-') || _istalnum(*pstr)) ) pstr = ::CharNext(pstr);
}
