...
namespace DuiLib {
	...
	bool DrawImage(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, const CDuiString& sImageName, const CDuiString& sImageResType, RECT rcItem, RECT rcBmpPart, RECT rcCorner, DWORD dwMask, UINT uFade, UINT uRotate, bool bGdiplus, bool bHole, bool bTiledX, bool bTiledY, HINSTANCE instance = NULL)
	{
		...
		data = pManager->GetImageEx(sImageName.GetData(), sImageResType.IsEmpty() ? NULL : sImageResType.GetData(), dwMask, false, bGdiplus, instance);
	...
	void CRenderEngine::DrawImage(HDC hDC, HBITMAP hBitmap, const RECT& rc, const RECT& rcPaint, const RECT& rcBmpPart, const RECT& rcCorners, bool bAlpha, UINT uFade, bool hole, bool xtiled, bool ytiled)
	{
		...
		if( lpAlphaBlend && (bAlpha || uFade < 255) ) {
			BLENDFUNCTION bf = { AC_SRC_OVER, 0, (BYTE)uFade, AC_SRC_ALPHA };
			// middle
			if( !hole ) {
	...
	void CRenderEngine::FillRoundRect(HDC hDC, const RECT& rc, int width, int height, DWORD dwColor)
	{
#ifdef USE_GDI_RENDER
		ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
		int ew = 0, eh = 0;
					...
					case _T('a'):  // Link
						{
							...
							//}
							aColorArray.Add((LPVOID)(INT_PTR)clrColor);
							...
							if( pFontInfo->bUnderline == false ) {
								...
								if( hFont == NULL ) hFont = pManager->AddFont(g_iFontID, pFontInfo->sFontName.GetData(), pFontInfo->iSize, pFontInfo->bBold, true, pFontInfo->bItalic, pFontInfo->bStrikeout);
					...
					case _T('b'):  // Bold
						{
							...
							if( pFontInfo->bBold == false ) {
								...
								if( hFont == NULL ) hFont = pManager->AddFont(g_iFontID, pFontInfo->sFontName.GetData(), pFontInfo->iSize, true, pFontInfo->bUnderline, pFontInfo->bItalic, pFontInfo->bStrikeout);
								pFontInfo = pManager->GetFontInfo(hFont);
								aFontArray.Add(pFontInfo);
								pTm = &pFontInfo->tm;
								::SelectObject(hDC, pFontInfo->hFont);
					...
					case _T('f'):  // Font
						{
							...
							if( pstrTemp != pstrText ) {
								...
								if( hFont == NULL ) hFont = pManager->AddFont(g_iFontID, sFontName.GetData(), iFontSize, bBold, bUnderline, bItalic, bStrikeout);
						...
						{    
							...
							if( sName.IsEmpty() ) { // Italic
								...
								if( pFontInfo->bItalic == false ) {
									HFONT hFont = pManager->GetFont(pFontInfo->sFontName.GetData(), pFontInfo->iSize, pFontInfo->bBold, pFontInfo->bUnderline, pFontInfo->bStrikeout, true);
									if( hFont == NULL ) hFont = pManager->AddFont(g_iFontID, pFontInfo->sFontName.GetData(), pFontInfo->iSize, pFontInfo->bBold, pFontInfo->bUnderline, pFontInfo->bStrikeout, true);
									pFontInfo = pManager->GetFontInfo(hFont);
									aFontArray.Add(pFontInfo);
									pTm = &pFontInfo->tm;
									::SelectObject(hDC, pFontInfo->hFont);
	...
	HBITMAP CRenderEngine::GenerateBitmap(CPaintManagerUI* pManager, RECT rc, CControlUI* pStopControl, DWORD dwFilterColor)
	{
		...
		if (!bUseOffscreenBitmap) {
			...
			COwnedRenderContextScope renderScope(pManager, hPaintDC);
			pRoot->Paint(renderScope.GetContext(), rc, pStopControl);
		}
	...
	HBITMAP CRenderEngine::GenerateBitmap(CPaintManagerUI* pManager, CControlUI* pControl, RECT rc, DWORD dwFilterColor)
	{
		...
		{
			COwnedRenderContextScope renderScope(pManager, hPaintDC);
			pControl->Paint(renderScope.GetContext(), rc, NULL);
		}
	...
	void CRenderEngine::AdjustImage(bool bUseHSL, TImageInfo* imageInfo, short H, short S, short L)
	{
		...
		if( pDevice != NULL ) pDevice->InvalidateImageGpu(imageInfo);
	}
