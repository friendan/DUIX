...
namespace DuiLib
{
	...
	void CComboBoxUI::PaintStatusImage(IRenderContext& ctx)
	{
		if (m_sArrowImage.IsEmpty())
			CComboUI::PaintStatusImage(ctx);
		else
		{
			// get index
	...
	void CComboBoxUI::PaintText(IRenderContext& ctx)
	{
		...
		rcText.right -= m_nArrowWidth; // add this line than CComboUI::PaintText(IRenderContext& ctx)
		...
		if( m_iCurSel >= 0 ) {
			...
			if( pElement != NULL ) {
				...
				pControl->DoPaint(ctx, rcText, NULL);
