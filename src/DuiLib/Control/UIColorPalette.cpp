...
#define HSLUNDEFINED (HSLMAX*2/3)
	...
	*/
	...
	{
		int nSaveDC = ctx.SaveState();

		ctx.StretchBlit(m_MemDc, m_rcItem.left, m_rcItem.top, m_rcItem.right - m_rcItem.left, m_nPalletHeight, 0, 1, 360, 200);
		ctx.StretchBlit(m_MemDc, m_rcItem.left, m_rcItem.bottom - m_nBarHeight, m_rcItem.right - m_rcItem.left, m_nBarHeight, 0, 210, 200, m_nBarHeight);

		RECT rcCurSorPaint = { m_ptLastPalletMouse.x - 4, m_ptLastPalletMouse.y - 4, m_ptLastPalletMouse.x + 4, m_ptLastPalletMouse.y + 4 };
		ctx.DrawImageString(rcCurSorPaint, m_rcPaint, m_strThumbImage);
