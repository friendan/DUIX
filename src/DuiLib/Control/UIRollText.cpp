...
namespace DuiLib
{
	...
	void CRollTextUI::PaintText(IRenderContext& ctx)
	{
		...
		if( m_bShowHtml ) {
			int nLinks = 0;
			ctx.DrawHtmlText(rc, sText.GetData(), GetAdjustColor(dwColor), NULL, NULL, nLinks, m_iFont, uTextStyle);
		}
		else {
			ctx.DrawText(rc, sText.GetData(), GetAdjustColor(dwColor), m_iFont, uTextStyle);
		}
