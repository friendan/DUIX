...

{
	...
	VOID CALLBACK GifAnimExQueueTimerProc(PVOID lpParameter, BOOLEAN /*TimerOrWaitFired*/)

	{

		DuiLib::CGifAnimExUI* pSelf = static_cast<DuiLib::CGifAnimExUI*>(lpParameter);
		...
		::PostMessage(hWnd, DuiLib::UIMSG_GIFANIMEX_TICK, (WPARAM)pSelf, 0);
...

{
...

	{
...

		{
			...
			if( m_pOwer == NULL || m_pManager == NULL || m_nDelay <= 0 ) return;

			HANDLE hTimer = NULL;

			if( ::CreateTimerQueueTimer(&hTimer, NULL, GifAnimExQueueTimerProc,

				reinterpret_cast<PVOID>(m_pOwer),

				(UINT)m_nDelay, (UINT)m_nDelay, WT_EXECUTEDEFAULT) ) {
	...
	bool CGifAnimExUI::DoPaint(IRenderContext& ctx, const RECT& rcPaint, CControlUI* pStopControl)

	{

		if( !::IntersectRect( &m_rcPaint, &rcPaint, &m_rcItem ) ) return true;

		m_pImp->DrawFrame( ctx.GetDC(),rcPaint,m_rcItem);
