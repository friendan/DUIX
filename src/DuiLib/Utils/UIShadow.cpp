...
, m_bIsDisableShadow(false)
{
	...
	m_rcShadowCorner.right = 8;
	m_rcShadowCorner.bottom = 8;
}
...
void CShadowUI::Update(HWND hParent)
{
	...
	if (m_bIsImageMode) {
		...
		if(pDrawInfo != NULL) {
			...
			CGdiRenderContext localCtx(hMemDC, m_pManager);
			IRenderContext* pCtx = ResolveRenderContext(m_pManager, hMemDC, localCtx);
			pCtx->DrawImage(data, rcPaint, rcPaint, rcBmpPart, corner, 0xFF, true, false, false);
		}
	}
	else {
