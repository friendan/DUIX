...
public:
	...
	virtual LRESULT OnGetWPPageOK(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled)
	{
		...
		if(pPage != NULL)
		{
			...
			{
				CString sTabPanel;
				sTabPanel.Format(_T("wp_tabpanel@%s"), (LPCTSTR)pPage->sType);
				...
				if(pTabPanel == NULL)
				{
					...
					{
						CString sPanel;
						sPanel.Format(_T("wp_panel_%s_%d"), (LPCTSTR)pPage->sType, i);
	...
	virtual LRESULT OnGetWPInfoOK(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled)
	{
		...
		{
			CString sPanel;
			sPanel.Format(_T("wp_panel_%s_%d"), (LPCTSTR)pKey->sType, pKey->nPage);
			...
			if(pPanel != NULL)
			{
				...
				if(m_Data.GetBizhi(pKey->sType, pKey->nPage, vInfos))
				{
					for(int i = 0; i < vInfos.size(); i++)
					{
						...
						CString sButton;
						sButton.Format(_T("wp_button_%s_%d"), (LPCTSTR)pKey->sType, atoi(Info.id.c_str()));
