...
DUI_BEGIN_MESSAGE_MAP(CMainWnd, WindowImplBase)
	...
	DUI_ON_MSGTYPE(DUI_MSGTYPE_ITEMCLICK,CMainWnd::OnItemClick)
DUI_END_MESSAGE_MAP()
...
void CMainWnd::Notify( TNotifyUI &msg )
{
	if(msg.sType.CompareNoCase(DUI_MSGTYPE_SCROLL) == 0)
	{
		...
		CString sName = (LPCTSTR)msg.pSender->GetName();
	...
	else if(msg.sType.CompareNoCase(DUI_MSGTYPE_SELECTCHANGED) == 0)
	{
		...
		CString sName = (LPCTSTR)pTabBtn->GetName();
		if(sName == _T("wp_tabbtn") && pTabLayout)
		{
			CString sType = (LPCTSTR)pTabBtn->GetUserData();
			CString sTabPanel;
			sTabPanel.Format(_T("wp_tabpanel@%s"), (LPCTSTR)sType);
