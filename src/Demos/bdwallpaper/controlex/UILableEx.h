...
namespace DuiLib
{
	class CLabelIcon : public CLabelUI
	{
	public:
		CLabelIcon()
		{
	...
	public:
		void PaintText(IRenderContext& ctx)
		{
			...
			if( IsEnabled() ) {
				if( m_bShowHtml )
					ctx.DrawHtmlText(rc, sText, m_dwTextColor, \
					NULL, NULL, nLinks, m_iFont, m_uTextStyle);
				else
					ctx.DrawText(rc, sText, m_dwTextColor, \
					m_iFont, m_uTextStyle);
			}
			else {
				if( m_bShowHtml )
					ctx.DrawHtmlText(rc, sText, m_dwDisabledTextColor, \
					NULL, NULL, nLinks, m_iFont, m_uTextStyle);
				else
					ctx.DrawText(rc, sText, m_dwDisabledTextColor, \
					m_iFont, m_uTextStyle);
			}
		}
	};
