...
namespace DuiLib
{
	...
	public:
		virtual void PaintStatusImage(IRenderContext& ctx)
		{
			...
			// 绘制壁纸图片
			ctx.DrawImageString(rcItem, m_rcPaint, PrepareImage(), NULL);
			// 绘制状态
			...
			if(m_uButtonState & UISTATE_HOT)
			{
				...
				// 工具栏绘制
				ctx.DrawImageString(rcToolBar, m_rcPaint, m_sWPToolbarImage, NULL);
				ctx.DrawImageString(rcFav, m_rcPaint, m_sWPFavImage, NULL);
				ctx.DrawImageString(rcSet, m_rcPaint, m_sWPSetImage, NULL);
			}
		}
