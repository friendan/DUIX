...
public:
	...
	void PaintForeColor(IRenderContext& ctx)
	{
		...
		g.SetSmoothingMode(Gdiplus::SmoothingModeHighQuality);//潩潩?

		RECT rcPos = GetPos();
		
		Gdiplus::SolidBrush bkbrush(m_dwBackColor);
		g.FillPie(&bkbrush, (INT)rcPos.left, (INT)rcPos.top, (INT)(rcPos.right - rcPos.left), (INT)(rcPos.bottom - rcPos.top), (Gdiplus::REAL)0, (Gdiplus::REAL)360);

		Gdiplus::SolidBrush forebrush(m_dwForeColor);
		int nStartDegree = 90;
		int nSweepDegree = (int)(360.0f * (m_nValue * 1.0f / (m_nMax - m_nMin)));
		g.FillPie(&forebrush, (INT)rcPos.left, (INT)rcPos.top, (INT)(rcPos.right - rcPos.left), (INT)(rcPos.bottom - rcPos.top), (Gdiplus::REAL)nStartDegree, (Gdiplus::REAL)nSweepDegree);
		...
		rcCenter.right -= 10;
		rcCenter.bottom -= 10;
		g.FillPie(&coverbrush, (INT)rcCenter.left, (INT)rcCenter.top, (INT)(rcCenter.right - rcCenter.left), (INT)(rcCenter.bottom - rcCenter.top), (Gdiplus::REAL)0, (Gdiplus::REAL)360);
...
public:
	...
	void PaintBackgroundColor(IRenderContext& ctx)
	{
		...
		g.FillPie(&brush, (INT)rcPos.left, (INT)rcPos.top, (INT)(rcPos.right - rcPos.left), (INT)(rcPos.bottom - rcPos.top), (Gdiplus::REAL)0, (Gdiplus::REAL)360);
		g.ReleaseHDC(hDC);
	}
};
...
public:
	...
	void PaintForeColor(IRenderContext& ctx)
	{
		...
		int nStartDegree = 90;
		int nSweepDegree = (int)(360.0f * (m_nValue * 1.0f / (m_nMax - m_nMin)));
		g.FillPie(&brush, (INT)rcPos.left, (INT)rcPos.top, (INT)(rcPos.right - rcPos.left), (INT)(rcPos.bottom - rcPos.top), (Gdiplus::REAL)nStartDegree, (Gdiplus::REAL)nSweepDegree);

		Gdiplus::SolidBrush brushbk(0xFFFFFFFF);
		g.FillPie(&brushbk, (INT)(rcPos.left + 10), (INT)(rcPos.top + 10), (INT)(rcPos.right - rcPos.left - 20), (INT)(rcPos.bottom - rcPos.top - 20), (Gdiplus::REAL)0, (Gdiplus::REAL)360);
